...
`edge_conditions.csv` contains reproducible simulated conditions generated by
...
The generated file varies conditions by edge with a fixed random seed. Scenario
closures are restricted to non-backbone edges so randomization does not remove
the connectivity backbone. Change `DEFAULT_SEED` in
`generate_edge_conditions.py` when a different reproducible dataset is needed.
